...
export default function Home() {
  ...
  const fetchOldData = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to fetch old data");
  ...
  const fetchNewData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${BACKEND_URL}/api/button-data`);
    ...
    <div className="flex min-h-screen items-center justify-center bg-zinc-950 font-sans text-zinc-50 p-6 selection:bg-emerald-500/30">
      <main className="w-full max-w-4xl rounded-2xl border border-zinc-800 bg-zinc-900/50 p-8 shadow-2xl backdrop-blur-xl sm:p-12">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={loading}
                className="rounded-lg bg-zinc-800 px-4 py-2 text-xs font-semibold text-zinc-300 hover:bg-zinc-700 hover:text-white transition-colors border border-zinc-700 disabled:opacity-50"
              >
                Old data
              </button>
              <button
                ...
                disabled={loading}
                className="rounded-lg bg-emerald-500/10 px-4 py-2 text-xs font-semibold text-emerald-400 hover:bg-emerald-500/20 hover:text-emerald-300 transition-colors border border-emerald-500/20 disabled:opacity-50"
              >
                New data
              </button>
            </div>
            {loading && !data ? (
              ...
              <div className={`flex items-center gap-2 rounded-full border px-4 py-1.5 text-sm font-medium transition-colors duration-300 ${loading ? "border-emerald-500/10 bg-emerald-500/5 text-emerald-500/50" : "border-emerald-500/20 bg-emerald-500/10 text-emerald-400"}`}>
                <span className="relative flex h-2 w-2">
                  {!loading && <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75"></span>}
                  <span className={`relative inline-flex h-2 w-2 rounded-full ${loading ? "bg-emerald-500/50" : "bg-emerald-500"}`}></span>
                </span>
        ...
        {error && (
          <div className="mb-8 rounded-lg border border-red-900/50 bg-red-900/20 p-4 text-sm text-red-200">
            ...
            <p className="text-red-300">
              Could not reach the backend service ({BACKEND_URL}). Ensure it is running.
            </p>
          </div>
        )}
          ...
          {[
            ...
            >
              ...
              {loading && !data ? (
                ...
                <span className={`mt-2 text-xl font-semibold transition-colors duration-300 ${loading ? "text-zinc-500" : "text-zinc-100"}`}>
                  {stat.value}
                </span>
              )}
            </div>
        ...
        <div>
          ...
          <div className={`grid gap-4 transition-opacity duration-300 ${loading && data ? "opacity-30 blur-sm grayscale" : "opacity-100 blur-0 grayscale-0"}`}>
            {loading && !data ? (
              [1, 2, 3, 4].map((i) => (
                <div
                  key={i}
